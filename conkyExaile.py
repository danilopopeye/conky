...
class CommandLineParser:
    ...
    def __init__(self):
        ...
        self.parser.add_option("-d", "--datatype", dest="datatype", default="TI", type="string", metavar="DATATYPE", help=u"[default: %default] The data type options are: ST (status), CA (coverart), TI (title), AL (album), AR (artist), GE (genre), YR (year), TN (track number), FN (file name), LE (length), PP (current position in percent), PT (current position in time), VO (volume), RT (rating). Not applicable at command line when using templates.")
            ...
            if self.musicData == None:
                ...
                if self.testDBus(bus, 'org.exaile.Exaile'):
                    ...
                    try:
                        ...
                        coverart = "" #iface.GetTrackAttr("coverart")
                        ...
                        length = float(iface.GetTrackAttr('__length'))
...
def main():
    ...
    if options.version == True:
        print >> sys.stdout,"conkyExaile v.2.07"
    else:
